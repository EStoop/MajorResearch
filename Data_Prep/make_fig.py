...
#      Toombre Diagram
################################
def Toombre(V,UW, v_thin=vthin.value, v_thick=vthick.value, over_plot = None):
	
	x = np.arange(-800, 800)
	plt.hist2d(V, UW, bins=50000, norm=LogNorm())

	if over_plot != None:
		plt.scatter(extra_stars[0], extra_stars[1], color = 'orange', s=0.8)

	plt.plot(x,Escape(x), c='red')
	plt.plot(x,ThickDisk(x, v_thick), c = 'red')
	plt.plot(x,ThinDisk(x, v_thin), c='red')
	plt.xlim(-1000,550)
	plt.ylim(0,1600)
	plt.show()
...
def subplot_space_image(gc1, thin, thick, halo, v_thin, v_thick, savefig = False):
	# Make a space image 
	gc1.representation = 'cylindrical'
	R = gc1.rho # distance from galactic centre in pc
	z = gc1.z # height in pc
	...
	ax[0].set_title('Thin disk, vthin = {} km/s'.format(v_thin))
	ax[0].text(100, -0.95e4, '#stars = {}'.format(str(len(thin))))
	...
	ax[1].set_title('Thick disk, vthick = {} km/s'.format(v_thick))
	ax[1].text(100, -0.95e4, '#stars = {}'.format(str(len(thick))))
	
	#ax[2].hist2d(R, z, bins = 500, norm = LogNorm(), cmap='gray')
	ax[2].hist2d(R[halo], z[halo], bins =500, norm=LogNorm())
	...
	ax[2].set_title('Halo')
	ax[2].text(100, -0.95e4, '#stars = {}'.format(str(len(halo))))
	...
	if savefig: 
		plt.savefig('/home/stoop/disks/Major/Plots/Disk/Im{0}-{1}.png'.format(str(v_thin), str(v_thick)))
		plt.close(f)
	else: plt.show()
